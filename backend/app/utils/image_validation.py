...
logger = logging.getLogger("validra.validation")

ALLOWED_EXTENSIONS = {".jpg", ".jpeg", ".png", ".webp", ".bmp"}

# Strict decompression: prevent truncated or partial image parsing
...
async def validate_and_read_image(file: UploadFile) -> Tuple[bytes, str, str, str]:
    ...
    - Content-type in ALLOWED_IMAGE_TYPES
    - File size <= settings.MAX_UPLOAD_SIZE_BYTES (5MB) via Content-Length and stream checking
    - Non-empty byte payload
    - Image format verification via Pillow (including decompression bomb protection)

    Raises:
        HTTPException: 400 Bad Request, 413 Content Too Large
    """
    ...
    if not ext:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"File has no extension. Allowed extensions: {', '.join(sorted(ALLOWED_EXTENSIONS))}"
        )
    ...
    # Fast check Content-Length header if provided
    ...
    # Read bytes with chunking to enforce 5MB limit
